...
/**
 * Log-scale Graph Digitizer — single file (UX 동일, 핵심 3개 수정)
 * - 앵커가 있으면 배경이미지 드래그 이동 금지 (리사이즈/줌만 허용)
 * - Guides X 탭에 입력한 X값을 그대로 표에 표기
 * - Points 좌표 표를 시리즈별로 분리
 *
 * 그 외 UI/배치는 기존 버전과 동일합니다.
 */
...
export default function App() {
  ...
  // Coordinates 탭(오른쪽-하단 떠 있는 작은 패널)
  ...
  function drawGrid(ctx: CanvasRenderingContext2D) {
    const r = innerRect(), mm = tMinMax();
    if (!isFinite(mm.xmin) || !isFinite(mm.xmax) || !isFinite(mm.ymin) || !isFinite(mm.ymax) || mm.xmax <= mm.xmin || mm.ymax <= mm.ymin) { ctx.save(); ctx.fillStyle = "#9CA3AF"; ctx.font = "12px ui-sans-serif, system-ui"; ctx.fillText("Invalid axis range", r.x + r.w / 2, r.y + r.h / 2); ctx.restore(); return; }
  ...
  const onMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (dragRef.current.active && bgEditMode) {
      if (!(anchorMode === "custom" && customAnchors[activeBg])) {
  ...
  const onMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (inPlot(px, py)) {
      ...
      setSeries(arr => arr.map((s, i) => i === activeSeries ? { ...s, points: [...s.points, d].sort((a,b)=>a.x-b.x) } : s));
      setSelectedPoint(null);
    }
  };

  const onMouseUp = () => { dragRef.current.active = false; resizeRef.current.active = false; };
  const onMouseLeave = () => { hoverRef.current = { x: null, y: null }; setHoverHandle("none"); dragRef.current.active = false; resizeRef.current.active = false; };
  const onWheel = (e: React.WheelEvent<HTMLCanvasElement>) => {
    if (!bgEditMode) return; e.preventDefault(); const k = e.deltaY < 0 ? 1.05 : 0.95;
    setBgXform((cur) => { const n = [...cur] as [BgXf, BgXf]; const xf = n[activeBg]; const nsx = clampS(xf.sx * k); const nsy = clampS(xf.sy * (keepAspect ? k : k)); n[activeBg] = { ...xf, sx: keepAspect ? nsx : nsx, sy: keepAspect ? nsx : nsy }; return n; });
    setTick(t => (t + 1) & 0xffff);
  };

  /* ==== Guides 결과/표 (요청 2,3 반영) ==== */
  const xGuideRows = guideXs.flatMap(gx => {
    return series.map(s => ({ gx, series: s.name, y: yAtX(s.points, gx) }));
  });
  const yGuideRows = guideYs.flatMap(gy => {
    return series.map(s => ({ gy, series: s.name, x: xAtY(s.points, gy) }));
  });
    ...
    <div className="min-h-screen bg-gray-100 text-gray-800 font-sans antialiased">
      ...
      <main className="grid grid-cols-1 gap-8 p-8 lg:grid-cols-[480px,1fr]">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="space-y-5 text-base">
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-gray-200 pt-4">
                ...
                {/* Undo/Clear (요청은 최상단이었지만, UI 변경 최소화를 위해 기존 섹션 하단 유지) */}
        ...
        <div className="relative rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          {/* === 오른쪽-하단 좌표 패널 (UI 최소 변경, 요청 2/3 충족) === */}
          <div className="pointer-events-auto absolute bottom-3 right-3 w-[380px] max-w-[90%] rounded-xl border border-gray-200 bg-white/95 shadow-lg backdrop-blur">
            <button
              className="flex w-full items-center justify-between rounded-t-xl bg-gray-100 px-3 py-2 text-left text-sm font-semibold text-gray-700"
              onClick={() => setCoordOpen((v) => !v)}
            >
              <span>Coordinates</span>
              <span className="text-xs">{coordOpen ? "▼" : "▲"}</span>
            </button>
            {coordOpen && (
              <div className="max-h-[280px] overflow-auto p-3 text-sm">
                {/* Points — 시리즈별 분리 (요청 3) */}
                <div className="mb-3">
                  <div className="mb-1 text-xs font-bold text-gray-600">Points (by series)</div>
                  {series.map((s, idx) => (
                        ...
                        <table className="w-full">
                          <thead>
                            <tr className="text-gray-500">
                              ...
                              <th className="py-1 pr-2 text-left">X</th>
                              <th className="py-1 pr-0 text-left">Y</th>
                            </tr>
                          </thead>
                          <tbody>
                            {s.points.map((p, i) => (
                              <tr key={i} className="border-t">
                                ...
                                <td className="py-1 pr-2 font-mono">{fmtReal(p.x)}</td>
                                <td className="py-1 pr-0 font-mono">{fmtReal(p.y)}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                ...
                {/* Guides — X/Y 분리, X에는 '입력한 X' 그대로 표시 (요청 2) */}
                <div className="grid gap-3 sm:grid-cols-2">
                  <div>
                    <div className="mb-1 text-xs font-bold text-gray-600">Guides — X</div>
                    {xGuideRows.length === 0 ? (
                      <div className="text-gray-500">No X guides</div>
                    ) : (
                      <table className="w-full">
                        <thead>
                          <tr className="text-gray-500">
                            <th className="py-1 pr-2 text-left">X</th>
                            <th className="py-1 pr-2 text-left">Series</th>
                            <th className="py-1 pr-0 text-left">Y@X</th>
                          </tr>
                        </thead>
                        <tbody>
                          {xGuideRows.map((r, i) => (
                            <tr key={i} className="border-t">
                              <td className="py-1 pr-2 font-mono">{fmtReal(r.gx)}</td>
                              <td className="py-1 pr-2">{r.series}</td>
                              <td className="py-1 pr-0 font-mono">{fmtReal(r.y)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                  ...
                  <div>
                    <div className="mb-1 text-xs font-bold text-gray-600">Guides — Y</div>
                    {yGuideRows.length === 0 ? (
                      <div className="text-gray-500">No Y guides</div>
                    ) : (
                      <table className="w-full">
                        <thead>
                          <tr className="text-gray-500">
                            <th className="py-1 pr-2 text-left">Y</th>
                            <th className="py-1 pr-2 text-left">Series</th>
                            <th className="py-1 pr-0 text-left">X@Y</th>
                          </tr>
                        </thead>
                        <tbody>
                          {yGuideRows.map((r, i) => (
                            <tr key={i} className="border-t">
                              <td className="py-1 pr-2 font-mono">{fmtReal(r.gy)}</td>
                              <td className="py-1 pr-2">{r.series}</td>
                              <td className="py-1 pr-0 font-mono">{fmtReal(r.x)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
