 ...
 * UI/UX 개선 버전 v2 (여백, 가독성, 색상 조화)
 * - 전체적인 여백과 컴포넌트 간 간격을 늘려 시각적 편안함 제공
 * - 기본 폰트 크기를 키우고 굵기를 조절하여 가독성 향상
 * - 파란색과 주황색을 포인트 컬러로 사용하여 일관되고 조화로운 색상 시스템 적용
 */
...
export default function App() {
    ...
    try {
        ...
        if (guideXs.length) {
            ...
            for (const gx of guideXs) {
                ...
                series.forEach((s) => {
                    const y = yAtX(s.points, gx); if (y === null || !isFinite(y)) return;
        ...
        if (guideYs.length) {
            ...
            for (const gy of guideYs) {
                ...
                series.forEach((s) => {
                    const x = xAtY(s.points, gy); if (x === null || !isFinite(x)) return;
  ...
  const serialize = (): PresetV1 => ({ v: 1, axes: { xMin, xMax, yMin, yMax, xLog, yLog }, series, ui: { activeSeries }, connect: { connectLines, lineWidth, lineAlpha, smoothLines, smoothAlpha }, bg: { keepAspect, anchorMode, customAnchors, activeBg, showAB, opacityAB, xform: bgXform }, images: bgUrls.current, guidesX: guideXs, guidesY: guideYs, cross: { fromX: showCrossFromX, fromY: showCrossFromY } });
    ...
    <div className="min-h-screen bg-gray-50 text-gray-800 font-sans antialiased">
      ...
      <header className="sticky top-0 z-20 flex items-center justify-between border-b border-gray-200 bg-white/80 p-4 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-3 text-base">
          <button onClick={savePresetFile} className="rounded-lg bg-gray-100 px-4 py-2 hover:bg-gray-200">Save Preset</button>
          <button onClick={() => presetFileRef.current?.click()} className="rounded-lg bg-gray-100 px-4 py-2 hover:bg-gray-200">Load Preset</button>
          ...
          <button onClick={copyShareURL} className="rounded-lg bg-gray-100 px-4 py-2 hover:bg-gray-200">Copy URL</button>
          <div className="h-6 w-px bg-gray-300" />
          <button onClick={() => { let out = "series,x,y\n"; series.forEach(s => s.points.forEach(p => out += `${s.name},${p.x},${p.y}\n`)); const url = URL.createObjectURL(new Blob([out], { type: "text/csv" })); const a = document.createElement("a"); a.href = url; a.download = `points_${Date.now()}.csv`; a.click(); setTimeout(() => URL.revokeObjectURL(url), 0); }} className="rounded-lg bg-gray-100 px-4 py-2 hover:bg-gray-200">Export CSV</button>
      ...
      {/* 메인 레이아웃 */}
      <main className="grid grid-cols-1 gap-8 p-8 lg:grid-cols-[420px,1fr]">
        
        {/* 좌측 패널: 컨트롤 */}
        <aside className="flex flex-col gap-6">
          <ControlSection title="Axes & Guides">
            ...
            <div className="!mt-5 space-y-3 border-t border-gray-200 pt-4">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-gray-600">Guide X</span>
              ...
              <div className="flex items-center gap-2">
                <span className="font-semibold text-gray-600">Guide Y</span>
          ...
          <ControlSection title="Image Setup">
                ...
                <label className="flex items-center gap-3">
                 ...
                 <div className="col-span-2 !mt-3 border-t border-gray-200 pt-5">
                    <div className="flex items-center gap-4">
                      <button onClick={() => setBgEditMode(v => !v)} className={`w-full rounded-lg px-3 py-2 font-semibold ${bgEditMode ? "bg-orange-100 text-orange-800" : "bg-gray-200 text-gray-800"}`}>
          ...
          <ControlSection title="Series & Points">
            <div className="flex items-center gap-6">
              <span className="text-lg font-bold">Active:</span>
              <label className="flex items-center gap-2 text-lg"><input type="radio" className="h-5 w-5" name="series" checked={activeSeries === 0} onChange={() => setActiveSeries(0)} /> Series A</label>
              <label className="flex items-center gap-2 text-lg"><input type="radio" className="h-5 w-5" name="series" checked={activeSeries === 1} onChange={() => setActiveSeries(1)} /> Series B</label>
            </div>
            <div className="grid grid-cols-2 gap-4">
             ...
             <div className="!mt-5 grid grid-cols-2 gap-x-6 gap-y-4 border-t border-gray-200 pt-5">
                ...
                {smoothLines && (
                  <label className="col-span-2 flex items-center gap-3">Strength<input type="range" className="w-full" min={0} max={0.9} step={0.05} value={smoothAlpha} onChange={(e) => setSmoothAlpha(Number(e.target.value))} /></label>
                )}
        ...
        {/* 우측 패널: 캔버스 */}
        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
           <div className="mb-3 h-6 text-base text-gray-600">
