...
/**
 * Log-scale Graph Digitizer — single file (v3.2)
 * - Load preset fix (file + URL hash)
 * - BG edit cursor types (ew/ns/nwse/move)
 * - Drag + wheel zoom + arrow-keys move
 * - Undo/Redo (Ctrl/⌘+Z / Ctrl/⌘+Y)
 * - Left panel order: Axes→Image→Series
 * - Center canvas
 */
...
export default function App() {
  ...
  const snapshot = () => ({
    ...
    bgXform: JSON.parse(JSON.stringify(bgXform)),
    showAB: [...showAB],
    opacityAB: [...opacityAB],
    customAnchors: JSON.parse(JSON.stringify(customAnchors)),
    activeBg,
    guidesX: [...guideXs],
    guidesY: [...guideYs],
  ...
  const applySnapshot = (s) => {
    if (!s) return;
    setXMin(s.axes.xMin); setXMax(s.axes.xMax);
    setYMin(s.axes.yMin); setYMax(s.axes.yMax);
    setXLog(!!s.axes.xLog); setYLog(!!s.axes.yLog);
    setSeries(s.series);
    setBgXform(s.bgXform);
    setShowAB(s.showAB); setOpacityAB(s.opacityAB);
    setCustomAnchors(s.customAnchors);
    setActiveBg(s.activeBg);
    setGuideXs(s.guidesX); setGuideYs(s.guidesY);
    setShowCrossFromX(s.cross.fromX); setShowCrossFromY(s.cross.fromY);
  };
  const undo = () => {
    const cur = snapshot();
  ...
  const [size] = useState({ w: 1120, h: 640 });
  ...
  const onMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (bgEditMode) {
      ...
      if (h !== "none") { const d = drawRectAndAnchor(activeBg); resizeRef.current = { active: true, mode: h, ax: d.ax, ay: d.ay, fx: d.fx, fy: d.fy, baseW: d.baseW, baseH: d.baseH }; pushHistory(); return; }
      if (overImage(px, py)) { dragRef.current = { active: true, startX: px, startY: py, baseX: bgXform[activeBg].offX, baseY: bgXform[activeBg].offY }; pushHistory(); return; }
      return;
    }
  ...
  const applyPreset = (p: any) => {
    try {
      ...
      pushHistory();
      if (p.axes) {
        setXMin(p.axes.xMin); setXMax(p.axes.xMax);
        setYMin(p.axes.yMin); setYMax(p.axes.yMax);
        setXLog(!!p.axes.xLog); setYLog(!!p.axes.yLog);
      }
      if (Array.isArray(p.series)) setSeries(p.series);
      if (p.ui) setActiveSeries(p.ui.activeSeries ?? 0);
      if (p.connect) {
        setConnectLines(!!p.connect.connectLines);
        setLineWidth(p.connect.lineWidth ?? 1.6);
        setLineAlpha(p.connect.lineAlpha ?? 0.9);
        setSmoothLines(!!p.connect.smoothLines);
        setSmoothAlpha(p.connect.smoothAlpha ?? 0.35);
      }
      if (p.bg) {
    ...
    } catch (e) { console.warn("preset apply fail", e); notify("Invalid preset", "err"); }
  };
  ...
  const savePresetFile = async () => {
    ...
    try {
      ...
      const a = document.createElement("a"); a.href = url; a.download = `digitizer_preset_${Date.now()}.json`; document.body.appendChild(a); a.click(); a.remove();
  ...
  const loadPresetFromFile = (file: File | null) => {
    if (!file) return;
    const fr = new FileReader();
    fr.onload = () => { try { applyPreset(JSON.parse(String(fr.result || "{}"))); } catch { notify("Invalid preset", "err"); } };
    fr.readAsText(file);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 text-gray-800 font-sans antialiased">
      {/* 헤더 */}
      <header className="sticky top-0 z-20 flex items-center justify-between border-b border-gray-200 bg-white/80 px-6 py-4 backdrop-blur-sm">
        <h1 className="text-xl font-bold text-gray-900">Log-scale Graph Digitizer</h1>
        <div className="flex items-center gap-3 text-base">
          <button onClick={undo} className="rounded-lg bg-gray-200 px-4 py-2 hover:bg-gray-300" title="Ctrl/⌘+Z">Undo</button>
          <button onClick={redo} className="rounded-lg bg-gray-200 px-4 py-2 hover:bg-gray-300" title="Ctrl/⌘+Y">Redo</button>
          ...
          <button onClick={() => {
            ...
            const url = URL.createObjectURL(new Blob([out], { type: "text/csv" })); const a = document.createElement("a");
            a.href = url; a.download = `points_${Date.now()}.csv`; a.click(); setTimeout(() => URL.revokeObjectURL(url), 0);
          }} className="rounded-lg bg-gray-200 px-4 py-2 hover:bg-gray-300">Export CSV</button>
      ...
      {/* 레이아웃 */}
      <main className="grid grid-cols-1 gap-8 p-8 xl:grid-cols-[420px,1fr]">
        {/* 좌측 패널: Axes → Image → Series */}
          ...
          <AccordionSection title="Axes & Guides" isOpen={axesOpen} onToggle={() => setAxesOpen(v => !v)}>
            <div className="grid grid-cols-2 gap-x-6 gap-y-3">
              <label className="col-span-2 flex items-center gap-3 text-lg">
                <input type="checkbox" className="h-5 w-5" checked={xLog} onChange={(e) => { pushHistory(); setXLog(e.target.checked); }} />
                <span>X Log Scale</span>
              </label>
              <label className="flex items-center gap-3">X Min <input type="number" className="w-full rounded-md border border-gray-300 px-3 py-2" value={xMin} onChange={(e) => { pushHistory(); setXMin(Number(e.target.value)); }} /></label>
              <label className="flex items-center gap-3">X Max <input type="number" className="w-full rounded-md border border-gray-300 px-3 py-2" value={xMax} onChange={(e) => { pushHistory(); setXMax(Number(e.target.value)); }} /></label>
              <label className="col-span-2 mt-2 flex items-center gap-3 text-lg">
                <input type="checkbox" className="h-5 w-5" checked={yLog} onChange={(e) => { pushHistory(); setYLog(e.target.checked); }} />
                <span>Y Log Scale</span>
              </label>
              <label className="flex items-center gap-3">Y Min <input type="number" className="w-full rounded-md border border-gray-300 px-3 py-2" value={yMin} onChange={(e) => { pushHistory(); setYMin(Number(e.target.value)); }} /></label>
              <label className="flex items-center gap-3">Y Max <input type="number" className="w-full rounded-md border border-gray-300 px-3 py-2" value={yMax} onChange={(e) => { pushHistory(); setYMax(Number(e.target.value)); }} /></label>
            </div>
            ...
            <div className="!mt-5 space-y-3 border-t border-gray-200 pt-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <input className="flex-grow rounded-md border border-gray-300 px-3 py-2" placeholder="e.g., 1000"
                  value={guideInput} onChange={(e) => setGuideInput(e.target.value)}
                  onKeyDown={(e) => { if (e.key === "Enter") { const v = Number(guideInput); if (isFinite(v) && v > 0) { pushHistory(); setGuideXs(g => Array.from(new Set([...g, v]))); } } }} />
                <button className="rounded-md bg-gray-200 px-3 py-2 hover:bg-gray-300" onClick={() => { const v = Number(guideInput); if (isFinite(v) && v > 0) { pushHistory(); setGuideXs(g => Array.from(new Set([...g, v]))); } }}>Add</button>
                <button className="rounded-md bg-gray-200 px-3 py-2 hover:bg-gray-300" onClick={() => { pushHistory(); setGuideXs([]); }}>Clear</button>
              ...
              <div className="flex items-center gap-2">
                ...
                <input className="flex-grow rounded-md border border-gray-300 px-3 py-2" placeholder="e.g., 10"
                  value={guideYInput} onChange={(e) => setGuideYInput(e.target.value)}
                  onKeyDown={(e) => { if (e.key === "Enter") { const v = Number(guideYInput); if (isFinite(v) && v > 0) { pushHistory(); setGuideYs(g => Array.from(new Set([...g, v]))); } } }} />
                <button className="rounded-md bg-gray-200 px-3 py-2 hover:bg-gray-300" onClick={() => { const v = Number(guideYInput); if (isFinite(v) && v > 0) { pushHistory(); setGuideYs(g => Array.from(new Set([...g, v]))); } }}>Add</button>
                <button className="rounded-md bg-gray-200 px-3 py-2 hover:bg-gray-300" onClick={() => { pushHistory(); setGuideYs([]); }}>Clear</button>
                ...
                <div className="!mt-5 grid grid-cols-2 gap-x-6 gap-y-4">
                  ...
                  <div className="col-span-2">
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <button onClick={() => { pushHistory(); setCustomAnchors(cur => { const n = [...cur] as [CustomAnchor, CustomAnchor]; n[activeBg] = null; return n; }); }} className="w-full rounded-lg bg-gray-200 px-3 py-2 font-semibold text-gray-800">Clear Anchor</button>
                    </div>
                  </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
            ...
            <div className="space-y-5 text-base">
              ...
              <div className="!mt-5 flex items-center gap-4 border-t border-gray-200 pt-4">
                <button onClick={() => { pushHistory(); setSeries(arr => arr.map((s, i) => i === activeSeries ? { ...s, points: s.points.slice(0, -1) } : s)); }} className="rounded-lg bg-gray-200 px-4 py-2 font-semibold hover:bg-gray-300">Undo Last Point</button>
                <button onClick={() => { pushHistory(); setSeries(arr => arr.map((s, i) => i === activeSeries ? { ...s, points: [] } : s)); }} className="rounded-lg bg-gray-200 px-4 py-2 font-semibold text-red-700 hover:bg-red-100">Clear Active Series</button>
              </div>
            </div>
          </div>
        </aside>
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="overflow-hidden rounded-lg border border-gray-300">
            <canvas
              ...
              width={size.w}
              height={size.h}
              className="block touch-none select-none mx-auto"
              style={{ cursor }}
